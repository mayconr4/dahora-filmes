 ...
 * Retorna um array vazio em case de erro ou chave inecistente
 */
...
export async function salvarLista(lista: Filme[]): Promise<void> {
  try {
    // Salva a lista de filmes no storager em formato de string
...
export async function salvarFilmeFavorito(filme: Filme): Promise<boolean> {
  ...
  de favoritos. A funçã some() retorna true se encontrar pelo menos 1 item que satisfaça a condição. */
  if (favoritos.some((filmeExistente) => filmeExistente.id === filme.id)) {
    // Já esxite? Não Precisamos salvar de novo
    // Por isso, retornamos false para indicar que o filme NÃO FOI adicionsdo
    return false;
  }

  // Se chegou até aqui, é porqu este filme ainda não tinha sido salvo.
...
/**
 *chamar a função carregarRetornar Retornar a lista de favoritos
 */
...
/* Remover comppletamente do storage os favoritos salvos */
export async function apagarTodosFavoritos(): Promise<void> {
  try {
    await AsyncStorage.removeItem(STORAGE_FAVORITOS);
  } catch (error) {
    console.error("Erro ao apgar todos os favoritos: " + error);
  }
}
